...
public abstract class BsProductStatusBhv extends AbstractBehaviorWritable {
     ...
     * @exception SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public OptionalEntity<ProductStatus> selectByUniqueOf(String productStatusName) {
        return doSelectByUniqueOf(productStatusName, ProductStatus.class);
    }

    protected <ENTITY extends ProductStatus> OptionalEntity<ENTITY> doSelectByUniqueOf(String productStatusName, Class<ENTITY> entityType) {
        return createOptionalEntity(doSelectEntity(xprepareCBAsUniqueOf(productStatusName), entityType), productStatusName);
    }

    protected ProductStatusCB xprepareCBAsUniqueOf(String productStatusName) {
        assertObjectNotNull("productStatusName", productStatusName);
        ProductStatusCB cb = newMyConditionBean(); cb.acceptUniqueOf(productStatusName);
        return cb;
    }
     ...
     * @return The list of the column value. (NotNull, EmptyAllowed, NotNullElement)
     */
    public List<String> extractProductStatusNameList(List<ProductStatus> productStatusList) {
        return helpExtractListInternally(productStatusList, new InternalExtractCallback<ProductStatus, String>() {
            public String getCV(ProductStatus et) { return et.getProductStatusName(); }
        });
    }
