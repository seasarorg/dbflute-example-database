...
/**
 * The behavior of (商品のさまりびゅー)SUMMARY_PRODUCT as VIEW. <br />
 * <pre>
 * [primary key]
 *     PRODUCT_ID
 *
...
public abstract class BsSummaryProductBhv extends AbstractBehaviorWritable {
    ...
    protected SummaryProduct facadeSelectByPKValue(Long productId) {
        return doSelectByPK(productId, typeOfSelectedEntity());
    }

    protected <ENTITY extends SummaryProduct> ENTITY doSelectByPK(Long productId, Class<ENTITY> tp) {
        return doSelectEntity(xprepareCBAsPK(productId), tp);
    }

    protected <ENTITY extends SummaryProduct> OptionalEntity<ENTITY> doSelectOptionalByPK(Long productId, Class<ENTITY> tp) {
    ...
    protected <ENTITY extends SummaryProduct> ENTITY doSelectByPKWithDeletedCheck(Long productId, Class<ENTITY> tp) {
     ...
     */
    public List<Long> extractProductIdList(List<SummaryProduct> summaryProductList)
    ...
    protected void doInsert(SummaryProduct et, InsertOption<SummaryProductCB> op) {
        assertObjectNotNull("summaryProduct", et); prepareInsertOption(op); delegateInsert(et, op);
    }
    ...
    protected void doUpdate(SummaryProduct et, UpdateOption<SummaryProductCB> op) {
        assertObjectNotNull("summaryProduct", et); prepareUpdateOption(op); helpUpdateInternally(et, op);
    }
    ...
    protected void doInsertOrUpdate(SummaryProduct et, InsertOption<SummaryProductCB> iop, UpdateOption<SummaryProductCB> uop) {
        assertObjectNotNull("summaryProduct", et); helpInsertOrUpdateInternally(et, iop, uop);
    }
    ...
    protected void doDelete(SummaryProduct et, final DeleteOption<SummaryProductCB> op) {
        assertObjectNotNull("summaryProduct", et); prepareDeleteOption(op); helpDeleteInternally(et, op);
    }

    protected void prepareDeleteOption(DeleteOption<SummaryProductCB> op) { if (op != null) { assertDeleteOptionStatus(op); } }
    ...
    protected int doQueryInsert(QueryInsertSetupper<SummaryProduct, SummaryProductCB> sp, InsertOption<SummaryProductCB> op) {
        ...
        SummaryProduct et = newEntity(); SummaryProductCB cb = createCBForQueryInsert();
    ...
    protected int doQueryUpdate(SummaryProduct et, SummaryProductCB cb, UpdateOption<SummaryProductCB> op) {
        assertObjectNotNull("summaryProduct", et); assertCBStateValid(cb); prepareUpdateOption(op);
