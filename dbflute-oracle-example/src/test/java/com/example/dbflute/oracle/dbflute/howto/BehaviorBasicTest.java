...
import com.example.dbflute.oracle.dbflute.exbhv.pmbean.MemberChangedToWithdrawalForcedlyPmb;
...
public class BehaviorBasicTest extends UnitContainerTestCase {
    ...
    public void test_selectEntity() {
        ...
        // ## Assert ##
        log(member.toString());
        assertEquals((Long) 3L, member.getMemberId());
    ...
    public void test_selectEntityWithDeletedCheck() {
        // ## Arrange ##
        MemberCB cb = new MemberCB();
        cb.query().setMemberId_Equal(99999L);
    ...
    public void test_update() {
        // ## Arrange ##
        Member beforeMember = memberBhv.selectByPKValueWithDeletedCheck(3L);
        Long versionNo = beforeMember.getVersionNo();// 排他制御のためにVersionNoを取得

        Member member = new Member();
        member.setMemberId(3L);// 更新したい会員の会員ID
        member.setMemberName("testName");
        ...
        // ## Assert ##
        Member afterMember = memberBhv.selectByPKValueWithDeletedCheck(3L);
    ...
    public void test_delete() {
        ...
        Member member = new Member();
        member.setMemberId(3L);// 削除したい会員の会員ID
        member.setVersionNo(versionNo);// 排他制御カラムの設定
        ...
        // ## Assert ##
        try {
            memberBhv.selectByPKValueWithDeletedCheck(3L);
            fail();
        } catch (EntityAlreadyDeletedException e) {
            // OK
            log(e.getMessage());
    ...
    public void test_deleteNonstrictIgnoreDeleted() {
        // ## Arrange ##
        Member member = new Member();
        member.setMemberId(99999L);// 存在しない会員の会員ID(既に削除されたと仮定)
        ...
        // ## Assert ##
        try {
            memberBhv.selectByPKValueWithDeletedCheck(99999L);
            fail();
        } catch (EntityAlreadyDeletedException e) {
            // OK
            log(e.getMessage());
