...
public abstract class BsMemberWithdrawal extends AbstractEntity implements EntityDefinedCommonColumn {
    ...
    /**
     * [get] (退会理由コード)WITHDRAWAL_REASON_CODE: {CHAR(3), FK to WITHDRAWAL_REASON} <br />
    ...
    /**
     * [set] (退会理由コード)WITHDRAWAL_REASON_CODE: {CHAR(3), FK to WITHDRAWAL_REASON} <br />
     * 定型的な退会した理由を参照するコード。<br />
     * 何も言わずに退会する会員もいるので必須項目ではない。
    ...
    /**
     * [get] (退会理由入力テキスト)WITHDRAWAL_REASON_INPUT_TEXT: {CLOB(1048576)} <br />
    ...
    /**
     * [set] (退会理由入力テキスト)WITHDRAWAL_REASON_INPUT_TEXT: {CLOB(1048576)} <br />
     * 会員がフリーテキストで入力できる退会理由。<br />
     * もう言いたいこと言ってもらう感じ。
