...
public abstract class BsWhiteTargetBhv extends AbstractBehaviorWritable {
     ...
     * int count = whiteTargetBhv.<span style="color: #DD4747">selectCount</span>(cb);
     * </pre>
     ...
     * WhiteTargetCB cb = new WhiteTargetCB();
     * cb.query().setFoo...(value);
     * WhiteTarget whiteTarget = whiteTargetBhv.<span style="color: #DD4747">selectEntity</span>(cb);
     * if (whiteTarget != null) { <span style="color: #3F7E5E">// null check</span>
     *     ... = whiteTarget.get...();
     * } else {
     *     ...
     * }
    ...
    protected <ENTITY extends WhiteTarget> OptionalEntity<ENTITY> doSelectOptionalEntity(WhiteTargetCB cb, Class<ENTITY> tp) {
     ...
     * WhiteTargetCB cb = new WhiteTargetCB();
     * cb.query().setFoo...(value);
     * WhiteTarget whiteTarget = whiteTargetBhv.<span style="color: #DD4747">selectEntityWithDeletedCheck</span>(cb);
     ...
     * @param targetId : PK, NotNull, DECIMAL(16). (NotNull)
     * @return The entity selected by the PK. (NullAllowed: if no data, it returns null)
     * @exception EntityDuplicatedException When the entity has been duplicated.
     * @exception SelectEntityConditionNotFoundException When the condition for selecting an entity is not found.
     */
    public WhiteTarget selectByPKValue(Long targetId) {
        return doSelectByPK(targetId, WhiteTarget.class);
    }

    protected <ENTITY extends WhiteTarget> ENTITY doSelectByPK(Long targetId, Class<ENTITY> entityType) {
        return doSelectEntity(xprepareCBAsPK(targetId), entityType);
    }

    protected <ENTITY extends WhiteTarget> OptionalEntity<ENTITY> doSelectOptionalByPK(Long targetId, Class<ENTITY> entityType) {
        return createOptionalEntity(doSelectByPK(targetId, entityType), targetId);
    }
    ...
    public WhiteTarget selectByPKValueWithDeletedCheck(Long targetId) {
        return doSelectByPKWithDeletedCheck(targetId, WhiteTarget.class);
    }

    protected <ENTITY extends WhiteTarget> ENTITY doSelectByPKWithDeletedCheck(Long targetId, Class<ENTITY> entityType) {
        return doSelectEntityWithDeletedCheck(xprepareCBAsPK(targetId), entityType);
    }

    protected WhiteTargetCB xprepareCBAsPK(Long targetId) {
        assertObjectNotNull("targetId", targetId);
        WhiteTargetCB cb = newMyConditionBean(); cb.acceptPrimaryKey(targetId);
        return cb;
    }
     ...
     * ListResultBean&lt;WhiteTarget&gt; whiteTargetList = whiteTargetBhv.<span style="color: #DD4747">selectList</span>(cb);
    ...
    protected <ENTITY extends WhiteTarget> ListResultBean<ENTITY> doSelectList(WhiteTargetCB cb, Class<ENTITY> tp) {
        assertCBStateValid(cb); assertObjectNotNull("entityType", tp);
     ...
     * PagingResultBean&lt;WhiteTarget&gt; page = whiteTargetBhv.<span style="color: #DD4747">selectPage</span>(cb);
    ...
    protected <ENTITY extends WhiteTarget> PagingResultBean<ENTITY> doSelectPage(WhiteTargetCB cb, Class<ENTITY> tp) {
        assertCBStateValid(cb); assertObjectNotNull("entityType", tp);
        return helpSelectPageInternally(cb, tp, new InternalSelectPageCallback<ENTITY, WhiteTargetCB>() {
     ...
     * WhiteTargetCB cb = new WhiteTargetCB();
     * cb.query().setFoo...(value);
     * whiteTargetBhv.<span style="color: #DD4747">selectCursor</span>(cb, new EntityRowHandler&lt;WhiteTarget&gt;() {
    ...
    protected <ENTITY extends WhiteTarget> void doSelectCursor(WhiteTargetCB cb, EntityRowHandler<ENTITY> handler, Class<ENTITY> tp) {
        assertCBStateValid(cb); assertObjectNotNull("entityRowHandler", handler); assertObjectNotNull("entityType", tp);
        assertSpecifyDerivedReferrerEntityProperty(cb, tp);
        helpSelectCursorInternally(cb, handler, tp, new InternalSelectCursorCallback<ENTITY, WhiteTargetCB>() {
            public void callbackSelectCursor(WhiteTargetCB cb, EntityRowHandler<ENTITY> handler, Class<ENTITY> tp) { delegateSelectCursor(cb, handler, tp); }
            public List<ENTITY> callbackSelectList(WhiteTargetCB cb, Class<ENTITY> tp) { return doSelectList(cb, tp); }
        });
    }
     ...
     * <pre>
     * whiteTargetBhv.<span style="color: #DD4747">scalarSelect</span>(Date.class).max(new ScalarQuery() {
    ...
    protected <RESULT, CB extends WhiteTargetCB> SLFunction<CB, RESULT> doScalarSelect(Class<RESULT> tp, CB cb) {
        assertObjectNotNull("resultType", tp); assertCBStateValid(cb);
        cb.xsetupForScalarSelect(); cb.getSqlClause().disableSelectIndex(); // for when you use union
        return createSLFunction(cb, tp);
    }

    protected <RESULT, CB extends WhiteTargetCB> SLFunction<CB, RESULT> createSLFunction(CB cb, Class<RESULT> tp) {
     ...
     * <pre>
     * whiteTargetBhv.<span style="color: #DD4747">loadWhiteRefTargetList</span>(whiteTargetList, new ConditionBeanSetupper&lt;WhiteRefTargetCB&gt;() {
     ...
     * <span style="color: #3F7E5E">//});</span>
     * for (WhiteTarget whiteTarget : whiteTargetList) {
     *     ... = whiteTarget.<span style="color: #DD4747">getWhiteRefTargetList()</span>;
     * }
     * </pre>
     ...
     */
    public NestedReferrerLoader<WhiteRefTarget> loadWhiteRefTargetList(List<WhiteTarget> whiteTargetList, ConditionBeanSetupper<WhiteRefTargetCB> setupper) {
        xassLRArg(whiteTargetList, setupper);
        return doLoadWhiteRefTargetList(whiteTargetList, new LoadReferrerOption<WhiteRefTargetCB, WhiteRefTarget>().xinit(setupper));
    }
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerLoader<WhiteRefTarget> loadWhiteRefTargetList(WhiteTarget whiteTarget, ConditionBeanSetupper<WhiteRefTargetCB> setupper) {
        xassLRArg(whiteTarget, setupper);
        return doLoadWhiteRefTargetList(xnewLRLs(whiteTarget), new LoadReferrerOption<WhiteRefTargetCB, WhiteRefTarget>().xinit(setupper));
    }
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerLoader<WhiteRefTarget> loadWhiteRefTargetList(WhiteTarget whiteTarget, LoadReferrerOption<WhiteRefTargetCB, WhiteRefTarget> loadReferrerOption) {
        xassLRArg(whiteTarget, loadReferrerOption);
        return loadWhiteRefTargetList(xnewLRLs(whiteTarget), loadReferrerOption);
    }
    ...
    public NestedReferrerLoader<WhiteRefTarget> loadWhiteRefTargetList(List<WhiteTarget> whiteTargetList, LoadReferrerOption<WhiteRefTargetCB, WhiteRefTarget> loadReferrerOption) {
        xassLRArg(whiteTargetList, loadReferrerOption);
        if (whiteTargetList.isEmpty()) { return (NestedReferrerLoader<WhiteRefTarget>)EMPTY_LOADER; }
        return doLoadWhiteRefTargetList(whiteTargetList, loadReferrerOption);
    }

    protected NestedReferrerLoader<WhiteRefTarget> doLoadWhiteRefTargetList(List<WhiteTarget> whiteTargetList, LoadReferrerOption<WhiteRefTargetCB, WhiteRefTarget> option) {
        final WhiteRefTargetBhv referrerBhv = xgetBSFLR().select(WhiteRefTargetBhv.class);
        return helpLoadReferrerInternally(whiteTargetList, option, new InternalLoadReferrerCallback<WhiteTarget, Long, WhiteRefTargetCB, WhiteRefTarget>() {
            public Long getPKVal(WhiteTarget et)
            { return et.getTargetId(); }
            ...
            public Long getFKVal(WhiteRefTarget re) { return re.getTargetId(); }
    ...
    public List<Long> extractTargetIdList(List<WhiteTarget> whiteTargetList) {
        return helpExtractListInternally(whiteTargetList, new InternalExtractCallback<WhiteTarget, Long>() {
            public Long getCV(WhiteTarget et) { return et.getTargetId(); }
        });
    }
     ...
     * whiteTargetBhv.<span style="color: #DD4747">insert</span>(whiteTarget);
    ...
    protected void doInsert(WhiteTarget whiteTarget, InsertOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTarget", whiteTarget);
        prepareInsertOption(op);
        delegateInsert(whiteTarget, op);
    }

    protected void prepareInsertOption(InsertOption<WhiteTargetCB> op) {
        if (op == null) { return; }
     ...
     * whiteTarget.<span style="color: #DD4747">setVersionNo</span>(value);
     * try {
     *     whiteTargetBhv.<span style="color: #DD4747">update</span>(whiteTarget);
    ...
    protected void doUpdate(WhiteTarget whiteTarget, final UpdateOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTarget", whiteTarget);
        prepareUpdateOption(op);
        helpUpdateInternally(whiteTarget, new InternalUpdateCallback<WhiteTarget>() {
            public int callbackDelegateUpdate(WhiteTarget et) { return delegateUpdate(et, op); } });
    }

    protected void prepareUpdateOption(UpdateOption<WhiteTargetCB> op) {
        if (op == null) { return; }
    ...
    protected void doInesrtOrUpdate(WhiteTarget whiteTarget, final InsertOption<WhiteTargetCB> iop, final UpdateOption<WhiteTargetCB> uop) {
        helpInsertOrUpdateInternally(whiteTarget, new InternalInsertOrUpdateCallback<WhiteTarget, WhiteTargetCB>() {
            public void callbackInsert(WhiteTarget et) { doInsert(et, iop); }
            public void callbackUpdate(WhiteTarget et) { doUpdate(et, uop); }
    ...
    protected void doCreateOrModify(Entity et, InsertOption<? extends ConditionBean> iop, UpdateOption<? extends ConditionBean> uop) {
        if (iop == null && uop == null) { insertOrUpdate(downcast(et)); }
            ...
            iop = iop != null ? iop : new InsertOption<WhiteTargetCB>();
            uop = uop != null ? uop : new UpdateOption<WhiteTargetCB>();
     ...
     * whiteTarget.<span style="color: #DD4747">setVersionNo</span>(value);
     * try {
     *     whiteTargetBhv.<span style="color: #DD4747">delete</span>(whiteTarget);
     * } catch (EntityAlreadyUpdatedException e) { <span style="color: #3F7E5E">// if concurrent update</span>
     *     ...
     * }
     * </pre>
    ...
    protected void doDelete(WhiteTarget whiteTarget, final DeleteOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTarget", whiteTarget);
        prepareDeleteOption(op);
        helpDeleteInternally(whiteTarget, new InternalDeleteCallback<WhiteTarget>() {
            public int callbackDelegateDelete(WhiteTarget et) { return delegateDelete(et, op); } });
    }

    protected void prepareDeleteOption(DeleteOption<WhiteTargetCB> op) {
        if (op == null) { return; }
     ...
     * }
     * whiteTargetBhv.<span style="color: #DD4747">batchInsert</span>(whiteTargetList);
     * </pre>
    ...
    public int[] batchInsert(List<WhiteTarget> whiteTargetList) {
        InsertOption<WhiteTargetCB> op = createInsertUpdateOption();
        return doBatchInsert(whiteTargetList, op);
    }

    protected int[] doBatchInsert(List<WhiteTarget> whiteTargetList, InsertOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTargetList", whiteTargetList);
        prepareBatchInsertOption(whiteTargetList, op);
        return delegateBatchInsert(whiteTargetList, op);
    }

    protected void prepareBatchInsertOption(List<WhiteTarget> whiteTargetList, InsertOption<WhiteTargetCB> op) {
        op.xallowInsertColumnModifiedPropertiesFragmented();
        op.xacceptInsertColumnModifiedPropertiesIfNeeds(whiteTargetList);
        prepareInsertOption(op);
    }
     ...
     *     whiteTargetList.add(whiteTarget);
     * }
     * whiteTargetBhv.<span style="color: #DD4747">batchUpdate</span>(whiteTargetList);
     * </pre>
    ...
    public int[] batchUpdate(List<WhiteTarget> whiteTargetList) {
        UpdateOption<WhiteTargetCB> op = createPlainUpdateOption();
        return doBatchUpdate(whiteTargetList, op);
    }

    protected int[] doBatchUpdate(List<WhiteTarget> whiteTargetList, UpdateOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTargetList", whiteTargetList);
        prepareBatchUpdateOption(whiteTargetList, op);
        return delegateBatchUpdate(whiteTargetList, op);
    }

    protected void prepareBatchUpdateOption(List<WhiteTarget> whiteTargetList, UpdateOption<WhiteTargetCB> op) {
        op.xacceptUpdateColumnModifiedPropertiesIfNeeds(whiteTargetList);
        prepareUpdateOption(op);
    }
     ...
     * whiteTargetBhv.<span style="color: #DD4747">batchUpdate</span>(whiteTargetList, new SpecifyQuery<WhiteTargetCB>() {
    ...
    protected int[] doBatchDelete(List<WhiteTarget> whiteTargetList, DeleteOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTargetList", whiteTargetList);
        prepareDeleteOption(op);
        return delegateBatchDelete(whiteTargetList, op);
    }
     ...
     * <pre>
     * whiteTargetBhv.<span style="color: #DD4747">queryInsert</span>(new QueryInsertSetupper&lt;WhiteTarget, WhiteTargetCB&gt;() {
    ...
    protected int doQueryInsert(QueryInsertSetupper<WhiteTarget, WhiteTargetCB> sp, InsertOption<WhiteTargetCB> op) {
        assertObjectNotNull("setupper", sp);
        prepareInsertOption(op);
        WhiteTarget e = new WhiteTarget();
        WhiteTargetCB cb = createCBForQueryInsert();
     ...
     * WhiteTargetCB cb = new WhiteTargetCB();
     * cb.query().setFoo...(value);
     * whiteTargetBhv.<span style="color: #DD4747">queryUpdate</span>(whiteTarget, cb);
     * </pre>
    ...
    protected int doQueryUpdate(WhiteTarget whiteTarget, WhiteTargetCB cb, UpdateOption<WhiteTargetCB> op) {
        assertObjectNotNull("whiteTarget", whiteTarget); assertCBStateValid(cb);
        prepareUpdateOption(op);
        return checkCountBeforeQueryUpdateIfNeeds(cb) ? delegateQueryUpdate(whiteTarget, cb, op) : 0;
    }

    @Override
    protected int doRangeModify(Entity et, ConditionBean cb, UpdateOption<? extends ConditionBean> op) {
        if (op == null) { return queryUpdate(downcast(et), (WhiteTargetCB)cb); }
        else { return varyingQueryUpdate(downcast(et), (WhiteTargetCB)cb, downcast(op)); }
    }
     ...
     * WhiteTargetCB cb = new WhiteTargetCB();
     * cb.query().setFoo...(value);
     * whiteTargetBhv.<span style="color: #DD4747">queryDelete</span>(whiteTarget, cb);
     * </pre>
     * @param cb The condition-bean of WhiteTarget. (NotNull)
    ...
    protected int doQueryDelete(WhiteTargetCB cb, DeleteOption<WhiteTargetCB> op) {
        assertCBStateValid(cb);
        prepareDeleteOption(op);
    ...
    protected int doRangeRemove(ConditionBean cb, DeleteOption<? extends ConditionBean> op) {
        if (op == null) { return queryDelete((WhiteTargetCB)cb); }
        else { return varyingQueryDelete((WhiteTargetCB)cb, downcast(op)); }
    }
     ...
     * whiteTargetBhv.<span style="color: #DD4747">varyingInsert</span>(whiteTarget, option);
     * ... = whiteTarget.getPK...(); <span style="color: #3F7E5E">// if auto-increment, you can get the value after</span>
     * </pre>
     * @param whiteTarget The entity of insert target. (NotNull, PrimaryKeyNullAllowed: when auto-increment)
     ...
     *     whiteTargetBhv.<span style="color: #DD4747">varyingUpdate</span>(whiteTarget, option);
     * } catch (EntityAlreadyUpdatedException e) { <span style="color: #3F7E5E">// if concurrent update</span>
     *     ...
     * }
     * </pre>
     ...
     * whiteTargetBhv.<span style="color: #DD4747">varyingQueryUpdate</span>(whiteTarget, cb, option);
     * </pre>
    ...
    protected <ENTITY extends WhiteTarget> void delegateSelectCursor(WhiteTargetCB cb, EntityRowHandler<ENTITY> rh, Class<ENTITY> tp)
    { invoke(createSelectCursorCBCommand(cb, rh, tp)); }
    protected <ENTITY extends WhiteTarget> List<ENTITY> delegateSelectList(WhiteTargetCB cb, Class<ENTITY> tp)
    { return invoke(createSelectListCBCommand(cb, tp)); }

    // -----------------------------------------------------
    //                                                Update
    //                                                ------
    protected int delegateInsert(WhiteTarget et, InsertOption<WhiteTargetCB> op)
    { if (!processBeforeInsert(et, op)) { return 0; }
      return invoke(createInsertEntityCommand(et, op)); }
    protected int delegateUpdate(WhiteTarget et, UpdateOption<WhiteTargetCB> op)
    { if (!processBeforeUpdate(et, op)) { return 0; }
      return delegateUpdateNonstrict(et, op); }
    protected int delegateUpdateNonstrict(WhiteTarget et, UpdateOption<WhiteTargetCB> op)
    { if (!processBeforeUpdate(et, op)) { return 0; }
      return invoke(createUpdateNonstrictEntityCommand(et, op)); }
    protected int delegateDelete(WhiteTarget et, DeleteOption<WhiteTargetCB> op)
    { if (!processBeforeDelete(et, op)) { return 0; }
      return delegateDeleteNonstrict(et, op); }
    protected int delegateDeleteNonstrict(WhiteTarget et, DeleteOption<WhiteTargetCB> op)
    { if (!processBeforeDelete(et, op)) { return 0; }
    ...
    protected int delegateQueryInsert(WhiteTarget et, WhiteTargetCB inCB, ConditionBean resCB, InsertOption<WhiteTargetCB> op)
    { if (!processBeforeQueryInsert(et, inCB, resCB, op)) { return 0; } return invoke(createQueryInsertCBCommand(et, inCB, resCB, op));  }
    protected int delegateQueryUpdate(WhiteTarget et, WhiteTargetCB cb, UpdateOption<WhiteTargetCB> op)
    ...
    //                                                                     Downcast Helper
    //                                                                     ===============
    protected WhiteTarget downcast(Entity et) {
        return helpEntityDowncastInternally(et, WhiteTarget.class);
    }
    ...
    @SuppressWarnings("unchecked")
    protected List<WhiteTarget> downcast(List<? extends Entity> ls) {
        return (List<WhiteTarget>)ls;
    }

    @SuppressWarnings("unchecked")
    protected InsertOption<WhiteTargetCB> downcast(InsertOption<? extends ConditionBean> op) {
        return (InsertOption<WhiteTargetCB>)op;
    }

    @SuppressWarnings("unchecked")
    protected UpdateOption<WhiteTargetCB> downcast(UpdateOption<? extends ConditionBean> op) {
        return (UpdateOption<WhiteTargetCB>)op;
    }

    @SuppressWarnings("unchecked")
    protected DeleteOption<WhiteTargetCB> downcast(DeleteOption<? extends ConditionBean> op) {
        return (DeleteOption<WhiteTargetCB>)op;
    }

    @SuppressWarnings("unchecked")
    protected QueryInsertSetupper<WhiteTarget, WhiteTargetCB> downcast(QueryInsertSetupper<? extends Entity, ? extends ConditionBean> sp) {
        return (QueryInsertSetupper<WhiteTarget, WhiteTargetCB>)sp;
    }
}
