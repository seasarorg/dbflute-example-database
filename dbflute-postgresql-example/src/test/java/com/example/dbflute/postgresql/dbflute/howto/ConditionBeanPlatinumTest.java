...
import com.example.dbflute.postgresql.dbflute.cbean.MemberAddressCB;
...
import com.example.dbflute.postgresql.dbflute.exentity.MemberAddress;
...
public class ConditionBeanPlatinumTest extends UnitContainerTestCase {
    ...
    public void test_configure_statementConfig() {
        // ## Arrange ##
        MemberCB cb = new MemberCB();
        // @jflute -- PostgreSQLのJDBCドライバがqueryTimeout()をサポートしていないようである
        // cb.configure(new StatementConfig().typeForwardOnly().queryTimeout(7).fetchSize(4).maxRows(3));
        cb.configure(new StatementConfig().typeForwardOnly().fetchSize(4).maxRows(3));
