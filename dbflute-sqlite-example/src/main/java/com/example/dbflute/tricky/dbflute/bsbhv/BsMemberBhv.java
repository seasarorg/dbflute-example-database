...
public abstract class BsMemberBhv extends AbstractBehaviorWritable {
     ...
     * @param conditionBeanSetupper The instance of referrer condition-bean set-upper for registering referrer condition. (NotNull)
     */
    public void loadMemberServiceList(Member member, ConditionBeanSetupper<MemberServiceCB> conditionBeanSetupper) {
        xassLRArg(member, conditionBeanSetupper);
        loadMemberServiceList(xnewLRLs(member), conditionBeanSetupper);
    }
    /**
     ...
     * <pre>
     * memberBhv.<span style="color: #FD4747">loadMemberServiceList</span>(memberList, new ConditionBeanSetupper&lt;MemberServiceCB&gt;() {
     ...
     * });
     * for (Member member : memberList) {
     *     ... = member.<span style="color: #FD4747">getMemberServiceList()</span>;
     * }
     * </pre>
     * About internal policy, the value of primary key(and others too) is treated as case-insensitive. <br />
     * The condition-bean that the set-upper provides have settings before you touch it. It is as follows:
     ...
     * @param conditionBeanSetupper The instance of referrer condition-bean set-upper for registering referrer condition. (NotNull)
     */
    public void loadMemberServiceList(List<Member> memberList, ConditionBeanSetupper<MemberServiceCB> conditionBeanSetupper) {
        xassLRArg(memberList, conditionBeanSetupper);
        loadMemberServiceList(memberList, new LoadReferrerOption<MemberServiceCB, MemberService>().xinit(conditionBeanSetupper));
    }
    /**
     * {Refer to overload method that has an argument of the list of entity.}
     * @param member The entity of member. (NotNull)
     * @param loadReferrerOption The option of load-referrer. (NotNull)
     */
    public void loadMemberServiceList(Member member, LoadReferrerOption<MemberServiceCB, MemberService> loadReferrerOption) {
        xassLRArg(member, loadReferrerOption);
        loadMemberServiceList(xnewLRLs(member), loadReferrerOption);
    }
    /**
     * {Refer to overload method that has an argument of condition-bean setupper.}
     * @param memberList The entity list of member. (NotNull)
     * @param loadReferrerOption The option of load-referrer. (NotNull)
     */
    public void loadMemberServiceList(List<Member> memberList, LoadReferrerOption<MemberServiceCB, MemberService> loadReferrerOption) {
        xassLRArg(memberList, loadReferrerOption);
        if (memberList.isEmpty()) { return; }
        final MemberServiceBhv referrerBhv = xgetBSFLR().select(MemberServiceBhv.class);
        helpLoadReferrerInternally(memberList, loadReferrerOption, new InternalLoadReferrerCallback<Member, Integer, MemberServiceCB, MemberService>() {
            public Integer getPKVal(Member et)
            { return et.getMemberId(); }
