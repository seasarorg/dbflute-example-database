package com.example.dbflute.tricky.dbflute.bsbhv;
...
import com.example.dbflute.tricky.dbflute.bsentity.dbmeta.*;
...
public abstract class BsMemberBhv extends AbstractBehaviorWritable {
     ...
     * <pre>
     * memberBhv.<span style="color: #DD4747">loadMemberServiceList</span>(memberList, new ConditionBeanSetupper&lt;MemberServiceCB&gt;() {
     ...
     * <span style="color: #3F7E5E">//});</span>
     * for (Member member : memberList) {
     *     ... = member.<span style="color: #DD4747">getMemberServiceList()</span>;
     * }
     * </pre>
     * About internal policy, the value of primary key (and others too) is treated as case-insensitive. <br />
     * The condition-bean, which the set-upper provides, has settings before callback as follows:
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerLoader<MemberService> loadMemberServiceList(List<Member> memberList, ConditionBeanSetupper<MemberServiceCB> setupper) {
        xassLRArg(memberList, setupper);
        return doLoadMemberServiceList(memberList, new LoadReferrerOption<MemberServiceCB, MemberService>().xinit(setupper));
    }
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerLoader<MemberService> loadMemberServiceList(Member member, ConditionBeanSetupper<MemberServiceCB> setupper) {
        xassLRArg(member, setupper);
        return doLoadMemberServiceList(xnewLRLs(member), new LoadReferrerOption<MemberServiceCB, MemberService>().xinit(setupper));
    }
     ...
     * @return The callback interface which you can load nested referrer by calling withNestedReferrer(). (NotNull)
     */
    public NestedReferrerLoader<MemberService> loadMemberServiceList(Member member, LoadReferrerOption<MemberServiceCB, MemberService> loadReferrerOption) {
        xassLRArg(member, loadReferrerOption);
        return loadMemberServiceList(xnewLRLs(member), loadReferrerOption);
    }
     ...
     */
    @SuppressWarnings("unchecked")
    public NestedReferrerLoader<MemberService> loadMemberServiceList(List<Member> memberList, LoadReferrerOption<MemberServiceCB, MemberService> loadReferrerOption) {
        xassLRArg(memberList, loadReferrerOption);
        if (memberList.isEmpty()) { return (NestedReferrerLoader<MemberService>)EMPTY_LOADER; }
        return doLoadMemberServiceList(memberList, loadReferrerOption);
    }

    protected NestedReferrerLoader<MemberService> doLoadMemberServiceList(List<Member> memberList, LoadReferrerOption<MemberServiceCB, MemberService> option) {
        final MemberServiceBhv referrerBhv = xgetBSFLR().select(MemberServiceBhv.class);
        return helpLoadReferrerInternally(memberList, option, new InternalLoadReferrerCallback<Member, Integer, MemberServiceCB, MemberService>() {
            public Integer getPKVal(Member et)
            { return et.getMemberId(); }
            ...
            public void qyFKIn(MemberServiceCB cb, List<Integer> ls)
            { cb.query().setMemberId_InScope(ls); }
            public void qyOdFKAsc(MemberServiceCB cb) { cb.query().addOrderBy_MemberId_Asc(); }
            public void spFKCol(MemberServiceCB cb) { cb.specify().columnMemberId(); }
            public List<MemberService> selRfLs(MemberServiceCB cb) { return referrerBhv.selectList(cb); }
            public Integer getFKVal(MemberService re) { return re.getMemberId(); }
            public void setlcEt(MemberService re, Member le)
            { re.setMember(le); }
